...
// Частота кадров по пути и при записи
const PATH_FPS = 15;
const VIDEO_FPS = 15;

// Количество «прогревочных» кадров до старта записи
...
async function loadScene(plyUrl) {
    ...
    // Рисуем в половинном разрешении для ускорения
    ...
    renderer.setPixelRatio(1); // без учёта devicePixelRatio
...
/**
 * Применение матрицы камеры из кадра path[index]
 */
...
/**
 * Основной цикл рендера, синхронизированный по PATH_FPS
 */
...
/**
 * «Прогрев» рендера до начала записи
 * (рендерим несколько кадров без MediaRecorder)
 */
...
export async function startRender(plyUrl, cameraPathUrl) {
    ...
    // Прогрев: несколько кадров без записи
